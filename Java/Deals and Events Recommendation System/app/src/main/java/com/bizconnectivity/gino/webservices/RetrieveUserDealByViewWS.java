...
import static com.bizconnectivity.gino.ConstantWS.WS_RETRIEVE_USER_DEAL_BY_NO_OF_VIEW;

public class RetrieveUserDealByViewWS {

    public static List<Deal> invokeRetrieveUserDealByView(int memberId) {

        List<Deal> dealList = new ArrayList<>();

        //create request
        SoapObject request = new SoapObject(NAMESPACE, WS_RETRIEVE_USER_DEAL_BY_NO_OF_VIEW);
        ...
        try {
            androidHttpTransport.call(SOAP_ACTION + WS_RETRIEVE_USER_DEAL_BY_NO_OF_VIEW, envelope);

            if (envelope.bodyIn instanceof SoapFault) {

                Log.d("TAG", "invokeRetrieveUserDealByView: " + envelope.bodyIn.toString());
