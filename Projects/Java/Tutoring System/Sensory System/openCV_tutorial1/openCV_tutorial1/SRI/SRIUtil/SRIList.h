...
#include "SRI/SRIUtil/SRIListIterator.h"
#include "SRI/SRIUtil/SRIConstListIterator.h"
#include <List>
...
namespace SRI{
   ...
   template<typename T> bool List<T>::operator==(const List& c) const{
	   if((m_ptStdList != NULL) && (c.m_ptStdList != NULL)){
		   if(m_ptStdList->size() != c.m_ptStdList->size()){
			   return false;
		   }
		   return (*m_ptStdList == *c.m_ptStdList);
		  /* StdListIterator a = m_ptStdList->begin();
		   StdListIterator b = c.m_ptStdList->begin();
		   while( (a != m_ptStdList->end()) && (b != c.m_ptStdList->end())){
			   if( *a != *b){
				   return false;
			   }
			   a++;
