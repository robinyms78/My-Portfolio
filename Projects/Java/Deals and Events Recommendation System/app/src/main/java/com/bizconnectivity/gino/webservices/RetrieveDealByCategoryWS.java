...
import static com.bizconnectivity.gino.ConstantWS.WS_RETRIEVE_DEAL_BY_CATEGORY;

public class RetrieveDealByCategoryWS {

    public static List<Deal> invokeRetrieveDealByCategory(int dealCategoryId) {

        List<Deal> dealList = new ArrayList<>();

        //create request
        SoapObject request = new SoapObject(NAMESPACE, WS_RETRIEVE_DEAL_BY_CATEGORY);

        PropertyInfo dealCategoryIdPI = new PropertyInfo();
        // Set Name
        dealCategoryIdPI.setName("dealCategoryId");
        // Set Value
        dealCategoryIdPI.setValue(dealCategoryId);
        // Set dataType
        dealCategoryIdPI.setType(int.class);
        // Add the property to request object
        request.addProperty(dealCategoryIdPI);
        ...
        try {
            androidHttpTransport.call(SOAP_ACTION + WS_RETRIEVE_DEAL_BY_CATEGORY, envelope);

            if (envelope.bodyIn instanceof SoapFault) {

                Log.d("TAG", "invokeRetrieveDealByCategory: " + envelope.bodyIn.toString());
                ...
                if (!soapObject.getProperty("diffgram").toString().equals("anyType{}")) {
                    ...
                    for (int i=0; i<newDataSet.getPropertyCount(); i++) {
                        ...
                        // 13: Eat, 14: Beauty, 15: Activities, 16: Services, 17: Travel
