...
import static com.bizconnectivity.gino.ConstantWS.WS_RETRIEVE_EVENT_FILTER;

public class RetrieveEventFilterWS {

    public static List<Event> invokeRetrieveNearbyEventFilter(String latitude, String longitude, String kilometer,
                                                              String startDate, String endDate, String name) {

        List<Event> eventModelList = new ArrayList<>();

        //create request
        SoapObject request = new SoapObject(NAMESPACE, WS_RETRIEVE_EVENT_FILTER);
        ...
        //property which holds input parameters
        PropertyInfo startDatePI = new PropertyInfo();
        //set name
        startDatePI.setName("startDate");
        //set value
        startDatePI.setValue(startDate);
        //set datatype
        startDatePI.setType(String.class);
        //add the property to request object
        request.addProperty(startDatePI);

        //property which holds input parameters
        PropertyInfo endDatePI = new PropertyInfo();
        //set name
        endDatePI.setName("endDate");
        //set value
        endDatePI.setValue(endDate);
        //set datatype
        endDatePI.setType(String.class);
        //add the property to request object
        request.addProperty(endDatePI);
        ...
        try {
            androidHttpTransport.call(SOAP_ACTION + WS_RETRIEVE_EVENT_FILTER, envelope);

            if (envelope.bodyIn instanceof SoapFault) {

                Log.d("TAG", "invokeRetrieveNearbyEventFilter: " + envelope.bodyIn.toString());
